import {
	...
	createSlice,
	PayloadAction,
	// current, Use for console logging, gets rid of proxy shit
...
function getAnswerCorrect(
	...
	if (!!previous && !!next) {
		return (
			currentQuestion.answer >= previous && currentQuestion.answer <= next
		);
	...
	(state) => {
		return state.activeQuestion && state.activeTeam
			? getAnswerCorrect(
					...
					state.teams[state.activeTeam!].timeline
			  )
			: false;
	}
);
	...
	reducers: {
		...
		addTeamMap(
			...
			action: PayloadAction<{
				[key: string]: { timeline: Question[]; color: string };
			}>
		) {
			state.teams = action.payload;
		},
		...
		answerQuestion(state, action: PayloadAction<string>) {
			...
			if (answerCorrect) {
				...
				state.teams[action.payload].timeline = teamArray.filter(
					(question) =>
						!state.timeline.some(
