import { Box, Button, Card, Checkbox, FormControl, FormControlLabel, FormHelperText, IconButton, InputLabel, MenuItem, Select, SelectChangeEvent, TextField, Typography, sliderClasses } from "@mui/material";
...
export default function Setup() {
    ...
    const addTeamOnClick = () => {
        ...
        dispatch(reducers.addTeam({ name: 'Team ' + teamIndex, color: 'default' }));
    };
    ...
    function handleTeamNameChange(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, index: number) {
        const newTeamName = event.target.value;
        dispatch(reducers.updateTeamName({ index: index, name: newTeamName }));
    };

    function handleTeamColorChange(event: SelectChangeEvent<string>, index: number) {
        const newTeamColor = event.target.value;
        dispatch(reducers.updateTeamColor({ index: index, color: newTeamColor }));
    };
    ...
    function nameTaken(name: string) {
        const nameCount = state.teams.filter(team => team.name === name).length;
        return nameCount > 1;
    };

    function handleSubmit () {
        if (state.teams.length === 0 || state.teams.some(team => team.name === '') || nameTaken('')) {
            return;
        }
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column' }}
                    ...
                    <Box sx={{ mb: 1 }}>
                        <FormControlLabel control={
                            <Checkbox 
                            // disabled 
                            checked={state.enableAi} onChange={handleAiChange} />
                    ...
                    <Box>
                        <Button startIcon={<AddIcon />} disabled={state.teams.length >= 6} onClick={addTeamOnClick} variant="outlined">
                            Add team
                        </Button>
                    </Box>
                    {state.teams.map((team, index) => (
                        <Box key={index} sx={{ mt: 1, display: 'flex' }}>
                            <TextField
                                ...
                                helperText={nameTaken(team.name) ? 'Duplicate name!' : ''}
                                variant="outlined"
                                value={team.name}
                            ...
                            />
                            <Select
                                sx={{ width: '50%', height: '100%', backgroundColor: team.color, borderRadius: '4px' }}
                                size="small"
                                value={team.color}
                                onChange={(event) => handleTeamColorChange(event, index)}
                                renderValue={(value) => {
                                    if (value === 'default') {
                                ...
                                <MenuItem value="default"><em>Select a color</em></MenuItem>
                                <MenuItem disabled={colorTaken('#BF616A')} sx={{ backgroundColor: '#BF616A' }} value={'#BF616A'}>Red</MenuItem>
                                <MenuItem disabled={colorTaken('#D08770')} sx={{ backgroundColor: '#D08770' }} value={'#D08770'}>Orange</MenuItem>
                                <MenuItem disabled={colorTaken('#EBCB8B')} sx={{ backgroundColor: '#EBCB8B' }} value={'#EBCB8B'}>Yellow</MenuItem>
                    ...
                    <Button fullWidth color="success" variant="contained" sx={{ mt: 1 }} type="submit">
                        Start game
                    </Button>
                </Box>
            </Card>
