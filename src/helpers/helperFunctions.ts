...
export function parseQuestions(str: string): Question[] {
	...
	while ((match = regex.exec(str)) !== null) {
		try {
			...
			console.error(
				`Error parsing question object: ${match[0]}. Skipping...`
			);
		}
	}
	return questions;
...
export function getGPTQuery(category: string) {
	return `"Generate 10 questions ${
		"about " + category
	} in the following format: {\"id\": <unique_id>,\"question\": \"In what year was <random_event>?\",\"answer\": <random_year>,\"description\": \"<random_event> took place in <random_year>.\"}\"`;
}
